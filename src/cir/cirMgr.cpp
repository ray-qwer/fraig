...
CirMgr::writeGate(ostream& outfile, CirGate *g) const
{
   //count PI AIG PO = 1
   Const0->setGlobalRef();
   vector<CirGate*> gate_pi_list;
   vector<CirGate*> gate_aig_list;
   vector<CirGate*> gate_dfs_list;
   g_dfs(g,gate_dfs_list);
   size_t max = 0;
   for(auto i : gate_dfs_list){
      if(i->getVar()>max)  max = i->getVar();
      if(i->getType()==PI_GATE)  gate_pi_list.push_back(i);
      else if (i->getType()==AIG_GATE)         gate_aig_list.push_back(i);
   }
   //start!!   
   sort(gate_pi_list.begin(),gate_pi_list.end(),compare_gate);
   outfile<<_type<<" "<<max<<" "<<gate_pi_list.size()<<" "<<0<<\
            " "<<1<<" "<<gate_aig_list.size()<<endl;  
   for (auto i : gate_pi_list) outfile << i->_var * 2 << endl;
   outfile<<g->_var*2<<endl;
   for (auto i : gate_aig_list) {
      if (i->_gateType == AIG_GATE) {
         outfile << i->_var * 2;
         for (size_t j = 0;j < i->_fanin.size(); ++j) {
            outfile << " " << i->_fanin[j].gate()->_var * 2 + int(i->_fanin[j].inv());
   ...
   for (size_t i = 0;i < gate_pi_list.size(); ++i) {
      if (gate_pi_list[i]->_symbo.size()) outfile << "i" << i << " " << gate_pi_list[i]->_symbo << endl;
   }
   outfile<<"o0 "<<g->getVar()<<endl;
   outfile << "c\n" << "Write gate ("<<g->_var<<") by Ray Hsu" << endl;
...
void
CirMgr::remove_from_aiglist(CirGate* _remove)
{
   assert(_remove->_gateType==AIG_GATE);
   for(auto i = _aiglist.begin();i!=_aiglist.end();++i){
      if(*i == _remove){
         _remove = 0;
         _aiglist.erase(i);
         break;
